...
def main():
    ...
    intf1 = ACI.Interface('eth', '1', '101', '1', '5')
    intf2 = ACI.Interface('eth', '1', '102', '1', '5')

    # Create a single VLAN for these interfaces.  Remember, EPGs do the allow-list
    vl15_intf1_web = ACI.L2Interface('vl15_intf1_web', 'vlan', '15')
    vl15_intf2_db = ACI.L2Interface('vl15_intf2_db', 'vlan', '15')

    # Attach the logical to physical interface config
    vl15_intf1_web.attach(intf1)
    vl15_intf2_db.attach(intf2)

    # Finally attach the EPGs to the layer-2 interface configs
    web_epg.attach(vl15_intf1_web)
    db_epg.attach(vl15_intf2_db) 
